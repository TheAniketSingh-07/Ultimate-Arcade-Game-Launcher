#!/usr/bin/env python3
"""
Optimized Game Launcher - High Performance Edition
Designed for smooth operation with minimal lag and maximum responsiveness
"""
...
WINDOW_WIDTH = 1200
WINDOW_HEIGHT = 800
TARGET_FPS = 120  # Higher FPS for smoother experience
VSYNC = True
...
class OptimizedGameLauncher:
    """Main launcher class with performance optimizations"""
    
    def __init__(self):
        # Initialize display with optimizations
        self.screen = pygame.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT), 
                                            pygame.DOUBLEBUF | pygame.HWSURFACE)
        pygame.display.set_caption("Optimized Game Launcher")
    ...
    def load_games(self) -> List[GameData]:
        ...
        game_configs = {
            "dino_run": GameData("Dino Run", "games/dino_run/dino_run.py", 
                               "Jump over obstacles!", "Action", (255, 100, 100), "dino_run.py"),
            "fighter_shoot": GameData("Fighter Shoot", "games/fighter_shoot/fighter_shoot.py",
                                    "Space combat action!", "Shooter", (100, 255, 100), "fighter_shoot.py"),
            "gravity_flip_ninja": GameData("Gravity Ninja", "games/gravity_flip_ninja/gravity_flip_ninja.py",
                                         "Flip gravity to survive!", "Platformer", (100, 100, 255), "gravity_flip_ninja.py"),
            "maze_game": GameData("Maze Explorer", "games/maze_game/maze_game.py",
                                "Navigate through mazes!", "Puzzle", (255, 255, 100), "maze_game.py"),
            "snake_game": GameData("Snake Classic", "games/snake_game/snake_game.py",
                                 "Classic snake gameplay!", "Classic", (255, 100, 255), "snake_game.py"),
            "tic_tac_toe": GameData("Tic Tac Toe", "games/tic_tac_toe/tic_tac_toe.py",
                                  "Classic strategy game!", "Strategy", (100, 255, 255), "tic_tac_toe.py")
        }
        
        for game_dir in os.listdir(games_dir):
            if game_dir in game_configs:
                game_path = os.path.join(games_dir, game_dir)
                if os.path.isdir(game_path):
                    games.append(game_configs[game_dir])
    ...
    def draw_header(self):
        ...
        # Title
        title_surface = self.renderer.get_text_surface("Arcade Game Launcher", 'title', Theme.TEXT_WHITE)
        title_rect = title_surface.get_rect(center=(WINDOW_WIDTH // 2, 40))
        self.screen.blit(title_surface, title_rect)
        
        # Subtitle
        subtitle_text = f"{len(self.games)} games available"
    ...
    def draw_performance_info(self):
        ...
        fps_surface = self.renderer.get_text_surface(fps_text, 'small', Theme.TEXT_GRAY)
    ...
    def launch_game(self, game_data: GameData):
        ...
        try:
            ...
            subprocess.Popen([sys.executable, game_data.path], 
                           cwd=os.path.dirname(game_data.path))
    ...
    def run(self):
        ...
        print("🚀 Optimized Game Launcher Started!")
